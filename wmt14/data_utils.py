# Adapted from tensorflow translation
...
# URLs for WMT data.
_WMT_ENFR_TRAIN_URL = "http://www-lium.univ-lemans.fr/~schwenk/cslm_joint_paper/data/bitexts.tgz"
_WMT_ENFR_DEV_URL = "http://www-lium.univ-lemans.fr/~schwenk/cslm_joint_paper/data/dev+test.tgz"
...
def get_wmt_enfr_train_set(directory):
    ...
    if not (gfile.Exists(train_path +".fr") and gfile.Exists(train_path +".en")):
        corpus_file = maybe_download(directory, "bitexts.tgz", _WMT_ENFR_TRAIN_URL)
        print("Extracting tar file %s" % corpus_file)
        bitexts = os.path.join(directory, "bitexts.selected")
        with tarfile.open(corpus_file, "r") as corpus_tar:
            corpus_tar.extractall(directory)
        for f in os.listdir(bitexts):
            if f.endswith(".en.gz"):
                gunzip_file(os.path.join(bitexts, f), train_path + ".en")
                gunzip_file(os.path.join(bitexts, f)[:-6]+".fr.gz", train_path + ".fr")
    return train_path


def get_wmt_enfr_dev_test_set(directory):
    """Download the WMT en-fr training corpus to directory unless it's there."""
    dev_name = "ntst1213"
    test_name = "ntst14"
    ...
    if not (gfile.Exists(dev_path + ".fr") and gfile.Exists(dev_path + ".en")):
        dev_file = maybe_download(directory, "dev+test.tgz", _WMT_ENFR_DEV_URL)
        ...
        with tarfile.open(dev_file, "r:gz") as dev_tar:
            ...
            fr_test_file = dev_tar.getmember("dev/" + test_name + ".fr")
            en_test_file = dev_tar.getmember("dev/" + test_name + ".en")
            fr_test_file.name = test_name + ".fr"  # Extract without "test/" prefix.
            en_test_file.name = test_name + ".en"
            dev_tar.extract(fr_test_file, directory)
            dev_tar.extract(en_test_file, directory)
...
def prepare_wmt_data(data_dir, en_vocabulary_size, fr_vocabulary_size, tokenizer=None, only_test=False):
    ...
    data_to_token_ids(test_path + ".fr", fr_test_ids_path, fr_vocab_path, tokenizer)
